...
async function loadHomepage(): Promise<
  | { ok: true; row: HomepageContentRow; previews: HomepageMediaPreviews }
...
> {
  ...
  if (mediaIds.length > 0) {
    const { data: mediaRows } = await supabase.from("media_uploads").select("id, public_url").in("id", mediaIds)
  ...
  return { ok: true, row: typed, previews }
}
