...
const PANEL_THEME: Record<
  string,
  { panel: string; icon: string; title?: string; body?: string }
> = {
  fuel: {
  ...
  restaurant: {
    panel: "bg-white",
    icon: "border-brand-border-accent bg-brand-surface-soft text-secondary",
    title: "text-brand-primary",
    body: "text-brand-body-soft",
  },
  market: {
...
export function HomeServiceColorBlocks({ blocks }: HomeServiceColorBlocksProps) {
  return (
    <Stagger once className="grid w-full grid-cols-1 overflow-hidden sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
      {blocks.map((block) => {
        ...
        return (
          ...
          >
            ...
            <h3
              className={cn(
                "font-(family-name:--font-montserrat) text-[1.2rem] font-extrabold tracking-[-0.02em] sm:text-[1.35rem]",
                theme.title
              )}
            >
              {block.title}
            ...
            <p
              className={cn(
                "mt-4 max-w-[18rem] text-[0.88rem] leading-[1.7] opacity-92 sm:mt-5 sm:max-w-[15rem] sm:text-[0.92rem] sm:leading-[1.75] lg:max-w-[16rem] xl:max-w-[18rem]",
                theme.body
              )}
            >
              {block.body}
